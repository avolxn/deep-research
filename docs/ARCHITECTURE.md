...
- SSE эндпоинты для real-time стриминга
...
- Стриминг событий от агента к клиенту
...
2. **Стриминг** → `GET /research/{id}/stream` запускает агента
3. **Уточнение** → Если нужно, агент возвращает `clarification` event
4. **Продолжение** → `POST /research/{id}/continue/stream`
5. **Исследование** → Супервизор делегирует задачи исследователям
6. **Отчёт** → `report` event с финальным Markdown
7. **Завершение** → `done` event, статус `completed`
